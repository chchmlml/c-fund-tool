$(document).ready(function () {
  ...
  function closeTab(menu, type) {
    ...
    var curTabIndex = $('#tabs').tabs('getTabIndex',
        $('#tabs').tabs('getTab', curTabTitle));
    switch (type) {
      case 1: // 刷新当前标签页
...
function showTime() {
  ...
  $("#clock").html(
      "现在是:" + this.year + "年" + this.month + "月" + this.date + "日 " + this.hour
...
function openTabs(node) {
  ...
  if (node.id === 6040) {
    ...
    $('#tabs').tabs('add', {// 添加新的标签页
      ...
      content: '<iframe scrolling="auto" height="99%" width="99.5%" frameborder="no" border="0"marginwidth="0" marginheight="0" scrolling="no" allowtransparency="yes" src="'
